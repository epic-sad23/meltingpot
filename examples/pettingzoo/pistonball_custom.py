...
class raw_env(AECEnv, EzPickle):
    ...
    def move_piston(self, piston, v):
        def cap(y):
            if y > self.maximum_piston_y:
                y = self.maximum_piston_y
            elif y < self.maximum_piston_y - (
    ...
    def step(self, action):
        ...
        if self._agent_selector.is_last():
            ...
            highest_pos = self.maximum_piston_y - 0.5 * self.pixels_per_position * self.n_piston_positions
            lowest_pos = self.maximum_piston_y # y vals go top to bottom
            middle_pos = (highest_pos+lowest_pos)/2
            def ramp_height(idx):
                ratio = idx/len(self.pistonList)
                return ratio*highest_pos + (1-ratio)*lowest_pos
            ...
            #total_reward = (
            #                [-np.var(list(map(lambda x: x.position[1], self.pistonList[0:2])))/100]
            #                +[-np.var(list(map(lambda x: x.position[1], self.pistonList[idx-1:idx+2])))/100 for idx in range(1,len(self.pistonList)-1)]
            #                +[-np.var(list(map(lambda x: x.position[1], self.pistonList[-1:-3:-1])))/100]
            #               )

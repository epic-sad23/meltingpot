...
def new_timestep_to_observations(timestep):
    gym_observations = {}
...
class _MeltingPotPettingZooEnv(pettingzoo_utils.ParallelEnv):
  ...
  def __init__(self, env_config, max_cycles):
    ...
    self.observation_space = functools.lru_cache(
        maxsize=None)(lambda agent_id: utils.spec_to_space(self._env.observation_spec()[0]['RGB']))
  ...
  def reset(self, seed=None, options=None):
    ...
    return new_timestep_to_observations(timestep), {}
  ...
  def step(self, action):
    ...
    observations = new_timestep_to_observations(timestep)
  ...
  def render(self, mode='not human why tf was that set as default', filename=None):
    """
